...
namespace UndertaleModTool
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged, IScriptInterface
    {
        ...
        public void CrashCheck()
        {
            try
            {
                ...
                if (Data == null && File.Exists(lastEditedLocation))
                {
                    ...
                    if (File.Exists(pathOfCrashedFile))
                    {
                        ...
                        {
                            if (OwnedMessageBox.Show("UndertaleModTool crashed during usage last time while editing " + pathOfCrashedFile + ", would you like to recover your code now?", "UndertaleModTool", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
                            {
                                LoadFile(pathOfCrashedFile, true).ContinueWith((t) => { });
                                if (Data == null)
                                {
                                    OwnedMessageBox.Show("Failed to load data when recovering.");
                                    return;
                                }
                                ...
                                OwnedMessageBox.Show("Completed.");
                            }
                            else if (OwnedMessageBox.Show("Would you like to move this code to the \"Recovered\" folder now? Any previous code there will be cleared!", "UndertaleModTool", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
                            {
                                OwnedMessageBox.Show("Your code can be recovered from the \"Recovered\" folder at any time.");
                            ...
                            {
                                OwnedMessageBox.Show("A crash has been detected from last session. Please check the Profiles folder for recoverable data now.");
                            }
                        }
                    }
                    else
            ...
            {
                OwnedMessageBox.Show("CrashCheck error! Send this to Grossley#2869 and make an issue on Github\n" + exc.ToString());
            }
        }
        ...
        public void ApplyCorrections()
        {
            ...
            {
                OwnedMessageBox.Show("ApplyCorrections error! Send this to Grossley#2869 and make an issue on Github\n" + exc.ToString());
            }
        }
        ...
        public void CreateUMTLastEdited(string filename)
        {
            ...
            {
                OwnedMessageBox.Show("CreateUMTLastEdited error! Send this to Grossley#2869 and make an issue on Github\n" + exc.ToString());
            }
        }
        ...
        public void DestroyUMTLastEdited()
        {
            ...
            {
                OwnedMessageBox.Show("DestroyUMTLastEdited error! Send this to Grossley#2869 and make an issue on Github\n" + exc.ToString());
            }
        }
        ...
        public void RevertProfile()
        {
            ...
            {
                OwnedMessageBox.Show("RevertProfile error! Send this to Grossley#2869 and make an issue on Github\n" + exc.ToString());
            }
        }
        public void SaveTempToMainProfile()
        {
            ...
            {
                OwnedMessageBox.Show("SaveTempToMainProfile error! Send this to Grossley#2869 and make an issue on Github\n" + exc.ToString());
            }
        }
        public void UpdateProfile(UndertaleData data, string filename)
        {
            try
            {
                ...
                if (SettingsWindow.ProfileModeEnabled)
                {
                    if (data.GMS2_3)
                    {
                        OwnedMessageBox.Show("Profile mode is not currently supported for GameMaker Studio 2.3 games.");
                        return;
                    }
                    ...
                    {
                        OwnedMessageBox.Show("Profile should exist, but does not. Insufficient permissions? Profile mode is disabled.");
                    ...
                    {
                        OwnedMessageBox.Show(@"The profile for your game loaded successfully!
...
an issue on GitHub.");
            ...
            {
                OwnedMessageBox.Show("UpdateProfile error! Send this to Grossley#2869 and make an issue on Github\n" + exc.ToString());
            }
        }
        public void ProfileSaveEvent(UndertaleData data, string filename)
        {
            try
            {
                ...
                else if (SettingsWindow.ProfileModeEnabled)
                {
                    ...
                    OwnedMessageBox.Show("Profile saved successfully to " + ProfileHash);
                }
            ...
            {
                OwnedMessageBox.Show("ProfileSaveEvent error! Send this to Grossley#2869 and make an issue on Github\n" + exc.ToString());
            }
        }
        public void DirectoryCopy(string sourceDirName, string destDirName, bool copySubDirs)
        {
            try
            {
                ...
                foreach (FileInfo file in files)
                {
                    ...
                    if (!File.Exists(tempPath))
                    {
                        ...
                        {
                            OwnedMessageBox.Show("An exception occurred while processing copying " + tempPath + "\nException: \n" + ex.ToString(), "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                            return;
                        }
                    }
                }
            ...
            {
                OwnedMessageBox.Show("DirectoryCopy error! Send this to Grossley#2869 and make an issue on Github\n" + exc.ToString());
            }
        }
    }
}
