...
// Bulk import by High Flyer using some code from ImportGML.csx
...
if (dirFiles.Length == 0)
    ...
    throw new ScriptException("The selected folder doesn't contain any JSON files.");

UndertaleRoom newRoom = new UndertaleRoom();
foreach (string file in dirFiles)
{
	newRoom = new UndertaleRoom();
	ReadRoom(Path.Combine(importFolder, file));
}
