...
foreach (var mod_dir in Directory.GetDirectories(mods_dir))
{
    string file_path;

    //-----------------------------------------------------
    string mod_embedded_textures = mod_dir + @"EmbeddedTextures\";
    ...
    foreach (file_path in Directory.GetFiles(mod_embedded_textures, @"*.png").Where(path => embedded_texture_file_regex.IsMatch(path)).ToList())
    {
    	try
    	{
	    	// get idx
    ...
    foreach (file_path in Directory.GetFiles(mod_sprites, @"*.png").Where(path => sprite_file_regex.IsMatch(path)).ToList())
    {
    	try
    	{
		    ...
		    //   if file name matches an existing sprite by name, and its frame number matches an existing texture page item index from said sprite
		    //     load png file as bitmap obj
		    //     load sprite's texture as bitmap obj
		    //     blit new sprite into texture
		    //     save texture
    ...
    //-----------------------------------------------------
    //string mod_audio =           mod_dir + @"Audio\";
    // TODO: overwrite game audio files with mod audio files
}
...
// TODO: foreach file in the EmbeddedTextures folder
foreach (var txtr in Data.EmbeddedTextures) 
{
    ...
    i++;
    ScriptMessage("Successfully imported " + import_count + " EmbeddedTextures");
}
...
Do you want to continue?");
    ...
    string importFolder = winFolder + "Sprites";
