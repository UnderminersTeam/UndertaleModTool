...
foreach (var mod_dir in Directory.GetDirectories(mods_dir))
{
    ...
    string file;
    Regex embedded_texture_regex = new Re

    // for each embedded texture
    foreach (file in Directory.GetFiles(mod_embedded_textures, "*.png").Where(path => reg.IsMatch(path)))
    {

    }

    irectory.GetFiles(yourPath, "*.png; *.jpg; *.gif")
                     .Where(path => reg.IsMatch(path))
                     .ToList();


    // foreach file in the Sprites folder
    //   parse file name "<SPRT>_#.png"
