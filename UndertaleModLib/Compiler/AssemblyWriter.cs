                    ...
                    if (compileContext.OriginalCode != null)
                    {
                        ...
                        if (locals != null)
                        {
                            ...
                            for (int i = 1; i < locals.Locals.Count; i++)
                            {
                                ...
                                if (compileContext.Data?.IsVersionAtLeast(2, 3) != true)
                            ...
                            compileContext.MainThreadDelegate.Invoke(() =>
                            {
                                ...
                                foreach (var l in compileContext.LocalVars)
                                {
                                    ...
                                    if (!hasLocal(name))
                                    {
                                        if (variables != null && compileContext?.Data.IsVersionAtLeast(2, 3) == true)
                                        {
                            ...
                            if (variables != null)
                            {
                                foreach (KeyValuePair<string, string> v in compileContext.LocalVars)
                                {
                                    ...
                                    if (def != null)
                                    {
                                        foreach (var patch in localPatches.FindAll(p => p.Name == v.Key))
                                        {
                                            ...
                                            else if (compileContext.Data.IsVersionAtLeast(2, 3))
                                ...
                                foreach (var patch in varPatches)
                                {
                                    if (patch.InstType != InstanceType.Local)
                                    {
                                        ...
                                        if (compileContext.Data.IsVersionAtLeast(2, 3) &&
                ...
                switch (s.Kind)
                {
                    ...
                    case Parser.Statement.StatementKind.With:
                        {
                            ...
                            if (cw.compileContext.Data.IsVersionAtLeast(2, 3) &&
                            ...
                            if (type != DataType.Int32)
                            {
                                if (cw.compileContext.Data.IsVersionAtLeast(2, 3) && type == DataType.Variable)
                    ...
                    case Parser.Statement.StatementKind.Return:
                        ...
                        else
                        {
                            ...
                            if (!(cw.compileContext.Data.IsVersionAtLeast(2, 3) && remaining == 1))
                                AssembleExit(cw);
                        }
                        break;
                ...
                if (e.Kind == Parser.Statement.StatementKind.ExprVariableRef)
                {
                    if (e.Children.Count == 1)
                    {
                        if (e.Children[0].Children.Count != 0)
                        {
                            ...
                            if (cw.compileContext.Data.IsVersionAtLeast(2, 3) && (cw.compileContext.BuiltInList.GlobalArray.ContainsKey(e.Children[0].Text) || cw.compileContext.BuiltInList.GlobalNotArray.ContainsKey(e.Children[0].Text)))
                            ...
                            if (cw.compileContext.Data.IsVersionAtLeast(2, 3) && e.Children[0].Children.Count > 2)
                            {
                            ...
                            if (duplicate)
                            {
                                if (cw.compileContext.Data.IsVersionAtLeast(2, 3) && e.Children[0].Children.Count != 1)
                                {
                                ...
                                if (cw.compileContext.BuiltInList.GlobalArray.ContainsKey(name) || cw.compileContext.BuiltInList.GlobalNotArray.ContainsKey(name))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        cw.varPatches.Add(new VariablePatch()
                                        {
                                            ...
                                            Name = name,
                                            InstType = (cw.compileContext.Data.IsVersionAtLeast(2, 3) && !useNoSpecificType) ? InstanceType.Builtin : InstanceType.Self,
                                            VarType = VariableType.Normal
                                        });
                                    }
                                }
                    ...
                    {
                        AssembleExpression(cw, e.Children[0]);
                        if (cw.compileContext.Data.IsVersionAtLeast(2, 3) && cw.typeStack.Peek() == DataType.Variable)
                        {
                            cw.typeStack.Pop();
                            cw.Emit(Opcode.PushI, DataType.Int16).Value = (short)-9; // stacktop conversion
                        }
                        ...
                        for (int next = 1; next < e.Children.Count; next++)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    cw.Emit(Opcode.Dup, DataType.Int32).Extra = (byte)(cw.compileContext.Data.IsVersionAtLeast(2, 3) ? 4 : 0);
                                }
                                cw.varPatches.Add(new VariablePatch()
                                {
                                    Target = cw.EmitRef(Opcode.Push, DataType.Variable),
            ...
            private static void AssembleStoreVariable(CodeWriter cw, Parser.Statement s, DataType typeToStore, bool skip = false, bool duplicate = false)
            {
                if (s.Kind == Parser.Statement.StatementKind.ExprVariableRef)
                {
                    if (s.Children.Count == 1)
                    {
                        ...
                        if (s.Children[0].Children.Count != 0)
                        {
                            ...
                            if (!skip)
                            {
                                ...
                                if (cw.compileContext.Data.IsVersionAtLeast(2, 3) && typeToStore != DataType.Variable)
                                {
                            ...
                            if (cw.compileContext.Data.IsVersionAtLeast(2, 3) && s.Children[0].Children.Count != 1)
                            {
                                if (duplicate)
                                {
                        ...
                        if (id >= 100000)
                            id -= 100000;
                        if (cw.compileContext.Data.IsVersionAtLeast(2, 3) && (cw.compileContext.BuiltInList.GlobalArray.ContainsKey(s.Children[0].Text) || cw.compileContext.BuiltInList.GlobalNotArray.ContainsKey(s.Children[0].Text)))
                        {
                    ...
                    else
                    {
                        ...
                        {
                            // Convert to variable in 2.3
                            if (cw.compileContext.Data.IsVersionAtLeast(2, 3) && typeToStore != DataType.Variable && s.Children.Last().Children.Count != 0)
                            {
                                cw.Emit(Opcode.Conv, typeToStore, DataType.Variable);
                                typeToStore = DataType.Variable;
                            }
